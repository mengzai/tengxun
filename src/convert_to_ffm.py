...
ad_feature=pd.read_csv(path1+'adFeature.csv')
user_feature=pd.read_csv(path1+'userFeature_split.data')

train=pd.read_csv(path1+'train.csv')
test=pd.read_csv(path1+'test2.csv')
print "load done"
...
user_ffm.to_csv(path1+'ffm.csv',index=False)
print 1
train = pd.read_csv(path1 + 'train.csv')
test = pd.read_csv(path1+'test2.csv')
print 2
...
with open('ffm.csv') as fin:
    f_train_out=open(path1+'train_ffm.csv','w')
    f_test_out = open(path1+'test_ffm.csv', 'w')
    print 4
